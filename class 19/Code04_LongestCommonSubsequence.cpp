...
//// 本题测试链接：https://leetcode.cn/problems/longest-common-subsequence/
int process1(string str1, string str2, int i, int j);

int longestCommonSubsequence(string text1, string text2) {
    if (text1.empty() || text2.empty() || text1.size() == 0 || text2.size() == 0) {
        return 0;
    }
...
int process1(string str1, string str2, int i, int j) {
    if (i == 0 && j == 0) {
        ...
        int p3 = str1[i] == str2[j] ? (1 + process1(str1, str2, i - 1, j - 2)) : 0;
        return max(p1, max(p2, p3));
    }
}

int longestCommonSubsequence1(string s1, string s2) {
    if (s1.empty() || s2.empty() || s1.size() == 0 || s2.size() == 0) {
        return 0;
    }
    int n = s1.size();
    int m = s2.size();
